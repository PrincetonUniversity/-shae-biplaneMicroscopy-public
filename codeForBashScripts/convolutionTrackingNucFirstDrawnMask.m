function [] = convolutionTrackingNucFirstDrawnMask(imageStructPath,fileStructPath,jFile,iiCell) 
...
maskPath = fullfile(fileparts (fileStruct.nucleoidParent{jFile}),'mask.png')
Imask =imread(maskPath);
ImaskBW = ~im2bw(Imask);
figure; imshow(ImaskBW,[])
IfMask= imfill(ImaskBW,'holes'); 
figure;imshow(IfMask,[])
...
IfMask = imtranslate(IfMask,[4, 7]); %empirically seen translation to match displacement of blue channel. 
ImaskBW= imtranslate(ImaskBW,[4, 7]);
...
for iiRow=1:7
    ...
    IfMask(iiRow,:) = IfMask(8,:);
    ImaskBW(iiRow,:) = ImaskBW(8,:);
end
for iiCol =1:4
    Imem(:,iiCol) = Imem(:,5);
    ...
    IfMask(:,iiCol) = IfMask(:,5);
    ImaskBW(:,iiCol) = ImaskBW(:,5);
end
...
figure;
imshow(double(maskCell ).* double(IfMask), [])
figure;
imshow(double(maskCell ).* double(ImaskBW), [])
%%
boxBot = [xBot,yBot];
...
edges = double(maskCell ).* double(ImaskBW);
...
BWdfill = imfill(BWsdil4,'holes'); %fill contours
