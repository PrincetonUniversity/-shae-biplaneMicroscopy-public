function [] = convolutionTrackingNucFirst(imageStructPath,fileStructPath,jFile,iiCell) 
...
Imem = imread(char(membraneList18(jFile)))+imread(char(membraneList19(jFile)));
Imem = Imem-2*PEDESTAL;
Imem = imtranslate(Imem,[4, 5]); %empirically seen translation to match displacement of blue channel. 
%imshow(Imem,[])
for iiRow=1:4
    Imem(iiRow,:) = Imem(6,:);
end
...
%bw(minBot(2):maxBot(2),minBot(1):maxBot(1),:)%
...
cellMask=imbinarize(cellMask1);
resMatNoConv = imageIn -0.55*max(imageIn(:))*fittedCellNoConv; 
